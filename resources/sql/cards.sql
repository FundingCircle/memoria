...
SELECT id, title, tags, contents, created_at FROM cards WHERE id IN (SELECT DISTINCT ON (ancestor_id) id FROM cards ORDER BY ancestor_id DESC, id DESC LIMIT :limit OFFSET :offset) ORDER BY id DESC;
...
SELECT id, title, tags, contents, created_at, ts_rank(tsv, tsquery, 1), created_at as rank FROM cards, to_tsquery(:query) tsquery WHERE tsquery @@ tsv GROUP BY tsquery, id HAVING id IN (SELECT DISTINCT ON (ancestor_id) id FROM cards ORDER BY ancestor_id DESC, id DESC) ORDER BY rank DESC, id DESC LIMIT :limit OFFSET :offset;

-- name: find-card-by-id
-- Find a card by id
SELECT id, title, tags, contents, ancestor_id, created_at FROM cards WHERE id = :id LIMIT 1;
